...
function Tessalator3D(data) {
  ...
  var i, uvs = [];
  var inner_radius = 0.999;  // point of inner position(should be set 0 ~ sphere's radius)
  for (i = 0; i < data.vertices.length; i += 2) {
    ...
    var wx = Math.cos(latRad) * Math.cos(lonRad);
    var wz = Math.cos(latRad) * Math.sin(lonRad);
    var wy = Math.sin(latRad);

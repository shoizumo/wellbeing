(() => {
  ...
  function init(vsMain, fsMain, vsPost, fsPost){
    ...
    let R = 3;
    geometry = new THREE.SphereBufferGeometry(R, 144, 144) ;
    ...
    let indices = new Uint16Array(geometry.index.array);
    for (let i = 0; i < indices.length; i ++ ) {
      indices[i] = i;
    }
    geometry.addAttribute('vertexId', new THREE.BufferAttribute(indices, 1));

...
void main(){
    vec4 bumpColor = texture2D(bumpTex, texCoord);
    vec4 landColor = texture2D(landTex, texCoord);
//    float bump = (3.0-(landColor.r + landColor.g + landColor.b)) * 0.5;// + bumpColor.r * 0.5;
//
    vUv = uv;
//    vec3 newPosition = position +  vec3(bump);
